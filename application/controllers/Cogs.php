...
class Cogs extends CI_Controller
{
    ...
    function delete_user($id = 0)
    {
        if ($id > 0) {
            ...
            if ($delete) {
                echo json_encode(['status' => 1, 'msg' => 'User deleted successfully']);
            } else {
                echo json_encode(['status' => 0, 'msg' => 'Failed to delete user']);
            }
        } else {
            echo json_encode(['status' => 0, 'msg' => 'Invalid user ID']);
        }
    }
