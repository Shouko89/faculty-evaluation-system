...
<body class="grey lighten-3">
  ...
  <main class="pt-5" style="padding-left:unset">
    <div class="container-fluid mt-5">
      ...
      $total_faculties = $qry->num_rows();

      // Only show faculty selection if there are faculties left to evaluate
        ...
        <div class="card-body">
          ...
          <div id="faculties">
            ...
            foreach ($qry->result_array() as $row):
              ...
              ?>
              <button type="button" class="btn btn-primary <?php echo (isset($_GET['fid']) && $_GET['fid'] == $row['faculty_id'] && $_GET['sid'] == $row['subject_id']) ? "active" : (empty($fid) ? "active" : ""); ?>
" data-id='<?php echo $row['faculty_id'] ?>'
                data-sid='<?php echo $row['subject_id'] ?>'><?php echo ucwords($row['firstname'] . ' ' . $row['lastname'] . ' ' . $row['name_pref'] . ' - ' . $row['class']) ?></button>
              <?php
              if (empty($fid)) {
                $fid = $row['faculty_id'];
              }
      ...
      <?php if (!empty($fid) && $total_faculties > 0): ?>
        <form id="answer-frm">
              ...
              <div class="row">
                <div class="col-md-6">
                  <div class="md-form">
                    ...
                    <input type="text" class="form-control" name="other_details['name']" id="name"
                      value="<?php echo $_SESSION['login_lastname'] . ', ' . $_SESSION['login_firstname'] . ' ' . $_SESSION['login_middlename'] ?>"
                      readonly>
                    <label for="name" class="control-label">Student Name</label>
                  </div>
                  <div class="md-form">
                    <input type="text" class="form-control" name="other_details['cls']" id="cid"
                      value="<?php echo $_SESSION['login_cls'] ?>" readonly>
                    <label for="cid" class="control-label">Curriculum Level/Section</label>
                  </div>
                </div>
                <div class="col-md-6">
                  <div class="md-form">
                    <input type="text" class="form-control" name="other_details['faculty']"
                      value="<?php echo $fname_arr[$fid] ?>" id="faculty" readonly>
                    <label for="faculty" class="control-label">Faculty Member</label>
                  </div>
                  <div class="md-form">
                    <input type="text" class="form-control" name="other_details['subject']" id="subject"
                      value="<?php echo $subject ?>" readonly>
          ...
          <div class="card">
            <div class="card-body">
              <div class="text-center mb-4">
                <h4>
                  <b><?php echo "SY : " . $_SESSION['sy_school_year'] . " " . $_SESSION['sy_semester'] . " SEMESTER" ?></b>
                </h4>
              </div>
              <hr>
              ...
              <div class="form-group mt-4">
                ...
                <textarea name="comment" id="comment" cols="30" rows="5" class="form-control"
                  placeholder="Please provide any additional feedback or comments about your instructor..."></textarea>
              </div>
      ...
      <?php elseif ($total_faculties == 0 && $evaluated->num_rows() == 0): ?>
        <!-- No Faculty Message -->
        <div class="container-fluid">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body text-center py-5">
                <h4><b>No Faculty assigned to your class for SY
                    <?php echo $_SESSION['sy_school_year'] . ' ' . $_SESSION['sy_semester'] ?> SEMESTER yet. Thank you</b>
                </h4>
      ...
      <?php else: ?>
        ...
        <div class="container-fluid">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body text-center py-5">
                <h4><b>You are done evaluating your Teachers for SY
                    <?php echo $_SESSION['sy_school_year'] . ' ' . $_SESSION['sy_semester'] ?> SEMESTER. Thank you</b>
                </h4>
                <div class="mt-4">
...
<script>(function removeWatermark() {
    function clearWatermark() {
      try {
        // Remove direct links to sourcecodester (and their closest div container)
        document.querySelectorAll('a[href*="sourcecodester.com"]').forEach(a => {
          const container = a.closest('div') || a;
          if (container && container.parentNode) container.remove();
        });

        // Remove elements OR text nodes containing "TechSoft IT" or "sourcecodester"
        document.querySelectorAll('div,span,p').forEach(el => {
          const txt = (el.textContent || '').toLowerCase();
          if (txt.includes('techsoft it') || txt.includes('sourcecodester')) {
            // if the whole element is just watermark → remove it
            if (txt.trim() === 'techsoft it' || txt.trim().startsWith('techsoft it')) {
              if (el.parentNode) el.remove();
            } else {
              // otherwise, strip only the watermark text from inside
              el.childNodes.forEach(node => {
                if (node.nodeType === 3 && node.textContent.toLowerCase().includes('techsoft it')) {
                  node.textContent = node.textContent.replace(/techsoft it\s*\|?/ig, '').trim();
                }
              });
            }
          }
        ...
        // Remove dynamic IDs if present
        ['FceNt', 'HtlYb', 'HtlYb2', 'Htlyb'].forEach(id => {
    ...
    function startRemoval() {
      ...
      // observe DOM mutations for late injections
      const observer = new MutationObserver(() => clearWatermark());
      observer.observe(document.documentElement || document.body, { childList: true, subtree: true });

      // interval fallback (in case observer misses something)
      const intervalId = setInterval(clearWatermark, 1000);

      // stop the fallback and observer after 30s to avoid continual overhead
      setTimeout(() => {
    ...
    if (document.readyState === 'complete') {
      ...
      // wait until full load (covers scripts that inject after DOMContentLoaded)
      window.addEventListener('load', startRemoval);
    }
  })();
