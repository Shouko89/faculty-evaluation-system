...
<!-- toast -->
  ...
  <main class="pt-5" style="padding-left:unset">
    ...
    <div id="faculties">
      ...
      ?>
        <button type="button" class="btn btn-primary btn-rounded <?php echo (isset($_GET['fid']) && $_GET['fid'] == $row['faculty_id'] && $_GET['sid'] == $row['subject_id']) ? "active" : (empty($fid) ? "active" : ""); ?>
" data-id='<?php echo $row['faculty_id'] ?>'  data-sid='<?php echo $row['subject_id'] ?>'><?php echo ucwords($row['firstname']. ' '. $row['lastname'].' '.$row['name_pref'].' - '.$row['class']) ?></button>
        <?php 
        if(empty($fid)){
          $fid = $row['faculty_id'];
        }
